...
// Pre-curated trending & 8D Beast Mode tracks with verified IDs
...
// Helper to scrape YouTube search results reliably
  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const item of items) {
        ...
        if (v && v.videoId) {
          const title =
            v.title?.runs?.map((r: any) => r.text).join("") || v.title?.simpleText || "Unknown Track";
          const author =
...
app.get("/api/search", async (req, res) => {
  ...
  // Fallback to filtering featured songs if network issue
...
// Download metadata endpoint (supplies offline storage hints & downloadable stream info)
